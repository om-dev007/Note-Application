  ...
  const handleForm = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setType("error");
        setMessage(data.message || "Error saving note");
  ...
  return (
    <div className="min-h-screen w-full bg-linear-to-br from-slate-950 via-indigo-950 to-slate-900 text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-8 flex justify-between items-center">
        ...
        <Link to="/notes" className="bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded-lg text-sm font-medium shadow-lg">📒 All Notes</Link>
      </div>
      ...
      <div className="flex justify-center px-6 py-10">
        ...
        >
          ...
          {message && (
            <div
              className={`mt-4 text-sm px-4 py-2 rounded-lg text-center ${
                type === "success"
          ...
          {type === "success" && (
            <Link to="/notes" className="w-full mt-4 bg-white/10 hover:bg-white/20 border border-white/20 py-2.5 rounded-lg text-sm">View All Notes →</Link>
          )}
        </form>
      </div>
    </div>
